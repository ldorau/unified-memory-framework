...
static tracker_alloc_info_t *
critnib_get_last_no_children(umf_memory_tracker_handle_t hTracker,
                             const void *ptr, int *_level, uintptr_t *_prev_key,
                             tracker_alloc_info_t **_prev_value) {
    assert(ptr);
    ...
    assert(!rvalue || rvalue->n_children == 0);
...
static umf_result_t umfMemoryTrackerRemove(umf_memory_tracker_handle_t hTracker,
    ...
    tracker_alloc_info_t *value = critnib_get_last_no_children(
