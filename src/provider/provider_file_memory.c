...
static umf_result_t file_initialize(void *params, void **provider) {
    ...
    size_t page_size;
    umf_result_t result = file_get_min_page_size(provider, NULL, &page_size);
    if (result != UMF_RESULT_SUCCESS) {
        return result;
    }
...
static inline void assert_is_aligned(uintptr_t ptr, size_t alignment) {
    (void)ptr;       // unused in Release build
    (void)alignment; // unused in Release build
