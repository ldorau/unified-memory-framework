...
static struct critnib_leaf *alloc_leaf(struct critnib *__restrict c) {
    ...
    // Verify if we are reusing a non-freed leaf.
    // In such case (extremely unlikely) a memory leak might happen.
    uint64_t ref_count;
    ...
    if (ref_count > 0) {
        ...
        if (to_be_freed != NULL) {
            ...
            LOG_DEBUG("WARNING: reusing probably non-freed leaf: key = %p, "
                      "value = %p, ref_count = %llu",
                      key, to_be_freed, (unsigned long long)ref_count);
            assert(ref_count == 0 && to_be_freed == NULL);
        }
    }
 ...
 *       critnib_remove() returns NULL if ref is NULL
 */
...
void *critnib_get(struct critnib *c, word key, void **ref) {
    ...
    if (res) {
        /*
         * Check if the value has just been removed (i.e. if ref_count == 0).
