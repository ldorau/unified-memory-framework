...
del_leaf:
    ...
    if (c->cb_free_leaf) {
        if (value == NULL) {
            fprintf(stderr,
                    "critnib_remove(): value == NULL (ptr = %llu, ref = %p, "
                    "k->to_be_freed = %p)\n",
...
int critnib_release(struct critnib *c, void *ref) {
...
#ifndef NDEBUG
    if (to_be_freed == NULL) {
        LOG_FATAL("leaf will not be freed (to_be_freed == NULL, ref = %p, "
                  "value = %p)\n",
                  ref, k->value);
    ...
    fprintf(stderr,
            "critnib_release(): ref_count == 0 for: ref_value=%p ptr=%p "
            "value=%p - free(to_be_freed=%p)\n",
