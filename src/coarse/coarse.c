...
static umf_result_t coarse_get_stats(coarse_t *coarse, coarse_stats_t *stats);

static bool debug_check(coarse_t *provider) {
    assert(provider);

    coarse_stats_t stats = {0};
    coarse_get_stats(provider, &stats);
...
static umf_result_t coarse_add_new_block(coarse_t *coarse, void *addr,
                                         size_t size) {
    block_t *new_block =
...
umf_result_t coarse_new(coarse_params_t *coarse_params, coarse_t **pcoarse) {
    ...
    if (coarse_params->upstream_memory_provider &&
        ...
        coarse_alloc(
            coarse, coarse_params->init_buffer_size, 0, &init_buffer);
        if (init_buffer == NULL) {
            goto err_destroy_mutex;
        }

        coarse_free(coarse, init_buffer,
                                    coarse_params->init_buffer_size);

    } else if (coarse_params->init_buffer) {
        umf_result =
            coarse_add_new_block(coarse, coarse->init_buffer,
                                 coarse_params->init_buffer_size);
        if (umf_result != UMF_RESULT_SUCCESS) {
            goto err_destroy_mutex;
        }
        ...
        LOG_DEBUG("coarse_ALLOC (init_buffer) %zu used %zu alloc %zu",
                  coarse_params->init_buffer_size, coarse->used_size,
                  coarse->alloc_size);
...
static int free_blocks_re_add(coarse_t *coarse, block_t *block) {
    ...
    if (node == NULL) {
        ...
        LOG_ERR("memory block not found (ptr = %p, size = %zu)",
...
umf_result_t coarse_add_memory_block(coarse_t *coarse, void *addr,
                                     size_t size) {
    umf_result_t umf_result;
    ...
    umf_result = coarse_add_new_block(coarse, addr, size);
...
umf_result_t coarse_alloc(coarse_t *coarse, size_t size, size_t alignment,
    ...
    if (action) { // ACTION_SPLIT or ACTION_USE
        ...
        if (alignment > 0) {
            ...
            if (umf_result != UMF_RESULT_SUCCESS) {
                free_blocks_re_add(coarse, curr);
                goto err_unlock;
            }
        }
    ...
    *resultPtr = NULL;
    ...
    umf_result = coarse_add_new_block(coarse, *resultPtr, size);
    if (umf_result != UMF_RESULT_SUCCESS && coarse->cb.free) {
...
umf_memory_provider_ops_t UMF_COARSE_MEMORY_PROVIDER_OPS = {
    ...
    .initialize = coarse_new,
    .finalize = coarse_delete,
    .alloc = coarse_alloc,
    .get_last_native_error = coarse_get_last_native_error,
    .get_recommended_page_size =
        coarse_get_recommended_page_size,
    .get_min_page_size = coarse_get_min_page_size,
    .ext.free = coarse_free,
    .ext.purge_lazy = coarse_purge_lazy,
    .ext.purge_force = coarse_purge_force,
    .ext.allocation_merge = coarse_merge,
    .ext.allocation_split = coarse_split,
    .ipc.get_ipc_handle_size = coarse_get_ipc_handle_size,
    .ipc.get_ipc_handle = coarse_get_ipc_handle,
    .ipc.put_ipc_handle = coarse_put_ipc_handle,
    .ipc.open_ipc_handle = coarse_open_ipc_handle,
    .ipc.close_ipc_handle = coarse_close_ipc_handle,
};
...
coarse_stats_t umfCoarseGetStats(coarse_t *coarse) {
    coarse_stats_t stats = {0};
