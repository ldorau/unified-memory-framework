...
static umf_result_t
utils_translate_mem_protection_one_flag(unsigned in_protection,
...
int utils_file_open_or_create(const char *path, size_t size) {
    ...
    if (ret) {
        LOG_PERR("setting size %zu of file %s failed", size, path);
    } else {
        LOG_DEBUG("set size of file %s to %zu bytes", path, size);
    }
